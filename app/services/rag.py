...
class RAGService:
    ...
    async def search(
        ...
        """
        accessible_group_ids = set(await group_service.user_group_ids(user_id))
        if group_id is not None:
            accessible_group_ids &= {group_id}
        if not accessible_group_ids:
            return []

        query_embedding = await self._embed_query(query)
        # pgvector's Vector comparator adds cosine_distance() at runtime via a custom
        # SQLAlchemy Comparator class, which pyright can't see through the Mapped[] type.
        distance = col(DocumentChunk.embedding).cosine_distance(query_embedding)  # pyright: ignore[reportAttributeAccessIssue]
