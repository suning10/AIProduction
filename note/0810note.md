...
- basically add hooks before and after executing LLM 
- provide tracebacks / cost 
...
graph.invoke(input, config)
  ...
  ┌─── LangChain Engine ───────────────────────────┐
  │                                                 │
  │   on_chain_start() ──→ Langfuse logs "started" │
  │         │                                       │
  │         ▼                                       │
  │   on_llm_start()  ──→ Langfuse logs prompt     │
  │         │                                       │
  │    [LLM EXECUTES]   ← actual work happens here  │
  │         │                                       │
  │   on_llm_end()    ──→ Langfuse logs response    │
  │         │                                       │
  │   on_chain_end()  ──→ Langfuse logs "finished"  │
  │                                                 │
  └─────────────────────────────────────────────────┘
```
...
### Route the Query to proper node 
...
- breaks down task to subtasks
- delegate subtasks to workers
- sync worker output into a final result
- eg.  a workflow that needs to update installation instructions for multiple Python libraries across an unknown number of documents might use this pattern
- ![img.png](img.png)
